...
def generate_medical_pdf(patient_history: str, audio_metrics: dict, blood_results: dict, filename: str, imaging_results: dict = None) -> str:
    """Generates an elite certified cardio-pulmonary & radiological diagnostic report PDF."""
    ...
    report_header_style = ParagraphStyle('RepHeader', fontName='Helvetica-Bold', fontSize=14, textColor=primary_navy, alignment=1, spaceAfter=10)
    sec_heading = ParagraphStyle('SecHead', fontName='Helvetica-Bold', fontSize=10.5, textColor=primary_navy, spaceBefore=10, spaceAfter=5)
    ...
    header_table = Table([
        [
            Paragraph("AISTETH CARDIO-PULMONARY & RADIOLOGY LAB<br/><font size=7>ISO 13485:2016 CERTIFIED INTEGRATED TELEHEALTH ARCHITECTURE / NABL: MC-99421</font>", lab_title),
            Paragraph(f"<b>REPORT ID:</b> MML-{int(time.time())}<br/><b>REGISTRATION DATE:</b> {time.strftime('%Y-%m-%d %H:%M')}<br/><b>STATUS:</b> COMPREHENSIVE FINAL RECORD", lab_subtitle)
        ]
    ], colWidths=[280, 260])
    ...
    story.append(Paragraph("INTEGRATED MULTI-MODAL DIAGNOSTIC TELEMETRY MATRIX", report_header_style))
    ...
    # 3. CARDIO-PULMONARY ACOUSTIC BLOCK
    story.append(Paragraph("2. ACOUSTIC FIELD ANALYTICS (PCG & PULMONARY VECTOR TRACKING)", sec_heading))
    acoustic_grid_data = [
    ...
    bl_table = Table(blood_table_data, colWidths=[150, 110, 140, 140])
    ...
    # 5. DYNAMIC IMAGING PANEL (X-RAY / MRI / CT SCAN MAPS)
    if imaging_results:
        story.append(Paragraph("4. INTEGRATED RADIOLOGICAL DIAGNOSTIC PANEL (COMPUTER VISION CORE)", sec_heading))
        img_status = imaging_results.get("imaging_status", "CLEAR")
        is_img_alert = "PATHOLOGICAL" in img_status.upper() or "🚨" in img_status
        
        imaging_grid_data = [
            [Paragraph("Radiological Assessment Parameter", table_header), Paragraph("Evaluated Finding / Analytics Context", table_header)],
            [Paragraph("Asset Spatial Dimensions", table_body_bold), Paragraph(imaging_results.get("asset_dimensions", "N/A"), table_body)],
            [Paragraph("Mean Structural Pixels Density", table_body_bold), Paragraph(str(imaging_results.get("mean_tissue_density", "N/A")), table_body)],
            [Paragraph("AI Vision Classification", table_body_bold), Paragraph(imaging_results.get("radiology_classification", "N/A"), table_body_alert if is_img_alert else table_body_bold)],
            [Paragraph("Framework Diagnostic Summary", table_body_bold), Paragraph(imaging_results.get("clinical_note", "N/A"), table_body)]
        ]
        
        img_table = Table(imaging_grid_data, colWidths=[160, 380])
        img_table.setStyle(TableStyle([
            ('BACKGROUND', (0,0), (-1,0), primary_navy), ('GRID', (0,0), (-1,-1), 0.5, border_gray),
            ('ROWBACKGROUNDS', (0,1), (-1,-1), [colors.white, bg_light]), ('VALIGN', (0,0), (-1,-1), 'TOP'),
    ...
    # 6. MASTER SYNTHESIS
    story.append(Paragraph("5. INTEGRATED PATHOLOGY SUMMARY & INTERPRETATION BRIEF", sec_heading))
    ...
    # 7. SIGN-OFF FOOTER
    footer_table = Table([[
        Paragraph("<b>Verification Clearance:</b> This certificate record is cryptographically mapped across standardized multi-modal health intelligence extraction frameworks.", ParagraphStyle('FN', fontName='Helvetica', fontSize=7, textColor=colors.HexColor("#64748B"))),
        Paragraph("<b>Digitally Signed By:</b><br/>Dr. Aris Thorne, MD, PhD<br/><font size=7 color='#64748B'>Chief Clinical Technical Director</font>", ParagraphStyle('SB', fontName='Helvetica-Bold', fontSize=8.5, textColor=primary_navy, alignment=2))
    ]], colWidths=[320, 220])
    footer_table.setStyle(TableStyle([('VALIGN', (0,0), (-1,-1), 'TOP'), ('LINEABOVE', (0,0), (-1,-1), 0.75, border_gray), ('TOPPADDING', (0,0), (-1,-1), 5)]))
